        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form" method="POST" action="#" enctype="multipart/form-data">
                    <?= csrf_field(); ?>
                    <input type="hidden" value="" id="id_album" name="id_album">
                    <input type="hidden" value="" id="foto_lm" name="foto_lm">
                    <div class="form-group row">
                        <label class="col-sm-4 col-form-label">Nama Album</label>
                        <div class="col-sm-8">
                            <input type="text" id="nm_album" name="nm_album" class="form-control form-control-sm" placeholder="Nama Album" required>
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        "responsive": true,
        "ajax": {
            "url": "<?= base_url(); ?>ladmin/album/ajax_list_album",
            "type": "POST"
        },
    });
    ...
    function edit(id) {
        ...
        $.ajax({
            url: "<?= base_url(); ?>/ladmin/album/edit/" + id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
                $('[name="id_album"]').val(data.id_album);
                $('[name="foto_lm"]').val(data.gambar);
                $('[name="nm_album"]').val(data.nm_album);
                ...
                if (data.gambar == '') {
                    ...
                    $('[name="foto_ev"]').attr('src', '<?= base_url() ?>/assets/images/album/' + data.gambar);
                    $('#foto_ev').show();
                }
                $('#modal_form').modal('show');
    ...
    function simpan() {
        ...
        if (save_method == 'tambah') {
            url = '<?= base_url(); ?>/ladmin/album/ajax_save';
        } else {
            url = '<?= base_url(); ?>/ladmin/album/ajax_update';
        }

        var id_album = $('#id_album').val();
        var foto_lm = $('#foto_lm').val();
        var nm_album = $('#nm_album').val();
        ...
        formData.append('nm_album', nm_album);
    ...
    function hapus_album(id) {
        alertify.confirm("Apakah Anda yakin akan menghapus data ini ?", function(ev) {
            ev.preventDefault();
            $.ajax({
                url: "<?= base_url(); ?>ladmin/album/ajax_delete/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
                    $('#modal_form').modal('hide');
