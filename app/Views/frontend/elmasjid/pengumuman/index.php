...
<div class="container-fluid pb-5 wow fadeInUp" data-wow-delay="0.1s">
    <div class="container">
        <div class="row g-4">
            <?php if ($jPengumuman == 0) {
                echo 'Belum ada Pengumuman';
            } else {
                $i = 0;
                foreach ($pengumuman as $bp) {
                    $i++;
            ?>
                    <div class="col-md-4 wow slideInUp mb-4" data-wow-delay="<?= (($i % 2) == 1) ? '0.1s' : '0.6s'; ?>">
                        <div class="blog-item bg-light rounded overflow-hidden">
                            <div class="p-4">
                                <a href="<?= base_url(); ?>pengumuman/<?= $bp['id_pengumuman']; ?>/<?= $bp['pengumuman_seo']; ?>">
                                    <h5 class="mb-2"><?= $bp['nm_pengumuman']; ?></h5>
                                </a>
                                <div class="mb-4">
                                    <small class="me-3"><i class="far fa-user text-primary me-2"></i><?= $bp['created_by']; ?></small>
                                    <small class="me-3"><i class="far fa-calendar-alt text-primary me-2"></i><?= tgl_indo($bp['created_at']); ?></small>
                                </div>
                                <p><?= substr(strip_tags($bp['isi_pengumuman']), 0, 150); ?>...</p>
                                <a href="<?= base_url(); ?>pengumuman/<?= $bp['id_pengumuman']; ?>/<?= $bp['pengumuman_seo']; ?>">Selengkapnya <i class="fas fa-arrow-right"></i></a>
                            </div>
                        </div>
                    </div>
            <?php
